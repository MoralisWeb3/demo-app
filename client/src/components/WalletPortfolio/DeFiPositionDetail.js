...
function DeFiPosition() {
  ...
  useEffect(() => {
    ...
    if (shouldFetchPosition) {
        ...
        fetch(`${process.env.REACT_APP_API_URL}/api/wallet/defi/positions/${globalDataCache.defiPosition.protocol.protocol_id}?chain=${globalDataCache.selectedChain}&wallet=${globalDataCache.walletAddress}`)
        .then(response => {
  ...
  return (
    <div id="defi-page" className="detailed">
...

      {
        protocol && (
            <>
      <button className="btn btn-sm btn-primary" onClick={goBack}>Back to DeFi Positions</button>
        <div className="wallet-card" key={protocol.name_name}>
                <div className="protocol-details">
                    ...
                    <div className="protocol-title">
                        ...
                        <spam className="rewards-available">+${utilities.formatPriceNumber(protocol.total_unclaimed_value_usd)} Unclaimed Rewards</spam>
                        )}
                ...
                {globalDataCache.defiPosition.positionDetail && (
                    ...
                    <ul className="defi-list">
                        ...
                        {globalDataCache.defiPosition.positionDetail.positions.map((position, index) => (
                            <div className="detailed-position">
                            <div className="position-detail" key={index}>
                                <>
                                    <h5>Position Summary - ${Number(position.balance_usd).toFixed(2)}</h5>
                                    ...
                                    <li key={index}>
                                        ...
                                        <>
                                            ...
                                            <div>
                                                {position.tokens.map((token,index) =>(
                                                    <>
                                                        <div>${utilities.formatPriceNumber(Number(token.usd_value))}</div> 
                                                    </>
                                                ))}
                                            </div>
                            ...
                            <li key={index}>
                                ...
                                {position.position_details.nft_metadata && (
                                    <div className="row">
                                        <p><b>{position.position_details.nft_metadata.name}</b></p>
                                        <p>{position.position_details.nft_metadata.description}</p>
                                        <div className="col">
                                            <img src={position.position_details.nft_metadata.image} />
                                        </div>
                                        <div className="col">
                                        ...
                                        <ul className="table-list">
                                            <li>
                                                ...
                                                <div className="right">{position.position_details.fee_tier}</div>
                                            </li>
                                            <li>
                                                <div className="left">Reserves</div>
                                                <div className="right">{position.position_details.reserves[0]}, {position.position_details.reserves[1]}</div>
                                            </li>
                                            <li>
                                                <div className="left">Liquidity</div>
                                                <div className="right">{position.position_details.liquidity}</div>
                                            </li>
                                            <li>
                                                <div className="left">Lower Price</div>
                                                <div className="right">{position.position_details.price_lower}  {position.position_details.price_label}</div>
                                            </li>
                                            <li>
                                                <div className="left">Upper Price</div>
                                                <div className="right">{position.position_details.price_upper}  {position.position_details.price_label}</div>
                                            </li>
                                            <li>
                                                <div className="left">Current Price</div>
                                                <div className="right">{position.position_details.current_price} {position.position_details.price_label}</div>
                                            </li>
                                            <li>
                                                <div className="left">In Range</div>
                                                <div className="right">{position.position_details.is_in_range ? "Yes" : "No"}</div>
                                            </li>
                                            <li>
                                                <div className="left">Pool Address</div>
                                                <div className="right">{utilities.shortAddress(position.position_details.pool_address)}</div>
                                            </li>
                                            <li>
                                                <div className="left">Position Key</div>
                                                <div className="right">{position.position_details.position_key}</div>
                                            </li>
                                        </ul>
                                        </div>
                                ...
                                {!position.position_details.fee_tier && (
                                    <ul className="table-list">
                                            ...
                                            <div className="right">
                                                {
                                                    ...
                                                    ? `${Number(position.position_details.apy*100).toFixed(4)}%`
                                                    : `0%`
                                                }
                                            </div>
                                        </li>
                                        <li>
                                            ...
                                            <div className="right">{position.position_details.isDebt ? "Yes" : "No"}</div>
                                        </li>
                                        <li>
                                            <div className="left">Variable Debt</div>
                                            <div className="right">{position.position_details.isVariableDebt ? "Yes" : "No"}</div>
                                        </li>
                                        <li>
                                            <div className="left">Stable Debt</div>
                                            <div className="right">{position.position_details.isStableDebt ? "Yes" : "No"}</div>
                                        </li>
                                        <li>
                                            <div className="left">Rewards Available</div>
                                            <div className="right">{protocol.total_unclaimed_value_usd ? "Yes" : "No"}</div>
                                        </li>
                                    </ul>
                                )}
                ...
                {!globalDataCache.defiPosition || !globalDataCache.defiPosition.positionDetail && <Skeleton />}
