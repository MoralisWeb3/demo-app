...
const WalletPnl = () => {
    ...
    const fetchPnl = async (chain, address) => {
        ...
        setError(false)
        try {
            const tokensResponse = await fetch(`${process.env.REACT_APP_API_URL}/api/wallet/pnl?chain=${chain ? chain : globalDataCache.selectedChain}&wallet=${address ? address : globalDataCache.walletAddress}`);
        ...
        {(globalDataCache.pnlLoaded && globalDataCache.pnl)  && (
          <>
                  ...
                  <div className="row">
                      <div className="col-lg-3">
                          <div className="wallet-card">
                              ...
                              <div className={Number(globalDataCache.pnl.total_realized_profit_percentage) < 0 ? "big-value negative" : "big-value positive"}>{Number(globalDataCache.pnl.total_realized_profit_percentage).toFixed(2)}%</div>
                          </div>
                      </div>
                      ...
                      <div className="col-lg-3">
                          <div className="wallet-card">
                              <div className="heading">Realized PnL $</div>
                              <div className={Number(globalDataCache.pnl.total_realized_profit_usd) < 0 ? "big-value negative" : "big-value positive"}>${Number(globalDataCache.pnl.total_realized_profit_usd).toFixed(2)}</div>
                          </div>
                      </div>
                      ...
                      <div className="col-lg-3">
                          <div className="wallet-card">
                              ...
                              <div className="big-value">${globalDataCache.pnl.total_trade_volume ? utilities.formatPriceNumber(globalDataCache.pnl.total_trade_volume) : 0}</div>
                          </div>
                      </div>
                      ...
                      <div className="col-lg-3">
                          <div className="wallet-card">
                              ...
                              <div className="big-value">{globalDataCache.pnl.total_count_of_trades}</div>
                          </div>
                      </div>
            ...
            <ul className="token-list">
              ...
              {globalDataCache.pnl && globalDataCache.pnl.tokens_traded.filter(token => token.realized_profit_usd !== "0").map(token => (
                  <li key={token.token_address} onClick={() => handleTokenClick(token)}>
                    ...
                    <div className={Number(token.realized_profit_usd) < 0 ? "negative" : "positive"} >${Number(token.realized_profit_usd).toFixed(2)}</div>
                    <div className={Number(token.realized_profit_usd) < 0 ? "negative" : "positive"} >{Number(token.realized_profit_percentage).toFixed(2)}%</div>
