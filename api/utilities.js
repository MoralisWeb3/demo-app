...
export const getChainExplorerUrl = (chainId) => {
  const explorers = {
  ...
  return explorers[chainId.toLowerCase()] || "Chain ID not recognized";
};
...
export const calcAge = (dob) => {
  ...
  if (age < 0) throw "Age is in the future!";
...
export const findAddressOccurrences = (transactions, currentUserAddress) => {
  ...
  transactions.forEach((transaction) => {
    const fromAddress =
      transaction.from_address_label || transaction.from_address;
    const toAddress = transaction.to_address_label || transaction.to_address;

    const normalizedFrom = transaction.from_address_label
      ? fromAddress
      : fromAddress.toLowerCase();
    const normalizedTo = transaction.to_address_label
      ? toAddress
      : toAddress.toLowerCase();

    // Count occurrences for 'from' addresses
    if (normalizedFrom !== normalizedUserAddress) {
      addressCounts.total[normalizedFrom] =
        (addressCounts.total[normalizedFrom] || 0) + 1;
      addressCounts.from[normalizedFrom] =
        (addressCounts.from[normalizedFrom] || 0) + 1;
    ...
    if (normalizedTo !== normalizedUserAddress) {
      addressCounts.total[normalizedTo] =
        (addressCounts.total[normalizedTo] || 0) + 1;
      addressCounts.to[normalizedTo] =
        (addressCounts.to[normalizedTo] || 0) + 1;

      uniqueSentToAddresses.add(normalizedTo);
    }
  });

  const sortedTotal = Object.entries(addressCounts.total).sort(
    ([, countA], [, countB]) => countB - countA
  );
  const sortedFrom = Object.entries(addressCounts.from).sort(
    ([, countA], [, countB]) => countB - countA
  );
  const sortedTo = Object.entries(addressCounts.to).sort(
    ([, countA], [, countB]) => countB - countA
  );
...
export const fetchPricesForChunk = async (tokensChunk, chain) => {
  try {
    ...
    const response = await fetch(
      `${baseURL}/erc20/prices?include=percent_change&chain=${chain}`,
      {
        method: "POST",
        headers: {
...
export async function enrichTransfersWithPrices(erc20_transfers, chain) {
  ...
  const uniqueTokenAddresses = Array.from(
    new Set(erc20_transfers.map((t) => t.address))
  ...
  for (const chunk of tokenChunks) {
    ...
    pricesChunk.forEach((priceInfo) => {
      currentPrices[priceInfo.tokenAddress.toLowerCase()] =
        priceInfo.usdPriceFormatted;
    });
  }
...
const fetchHistoricalPrice = async (tokenAddress, blockNumber, chain) => {
  try {
    const response = await fetch(
      `${baseURL}/erc20/${tokenAddress}/price?chain=${chain}&to_block=${blockNumber}
    `,
      {
        method: "GET",
        headers: {
    ...
    if (!response.ok) {
      ...
      console.error("Error response body:", errorBody);
      throw new Error(
        `Error fetching historical price: ${response.statusText}`
      );
    }
...
export async function getNativePrice(chain) {
  ...
  try {
    const response = await fetch(
      `${baseURL}/erc20/${tokenAddress}/price?chain=${chain}`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
...
export const fetchSinglePrice = async (tokenAddress, chain) => {
  try {
    const response = await fetch(
      `${baseURL}/erc20/${tokenAddress}/price?exchange=uniswapv3&chain=${chain}&include=percent_change`,
      {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
...
export async function enrichNFTTransactionsWithUSDValue(
  ...
  nftTransactions.forEach((tx) => {
    const priceData = historicalPrices.find(
      (price) =>
        price.to_block === tx.block_number &&
        price.token_address ===
          networkData.find((net) => net.id === tx.chain).wrappedTokenAddress
    );
  ...
  nftTransactions.forEach((tx) => {
    if (tx.historical_price) {
      ...
      tx.value_usd =
        ethers.utils.formatUnits(tx.value, 18) * tx.historical_price;
    } else {
      tx.value_usd = null;
    }
  });
...
export function formatPrice(price, precision = 2) {
  const roundedPrice = Number(
    Math.round(parseFloat(price + "e" + precision)) + "e-" + precision
  );
...
export function formatNumber(numberOrString) {
  ...
  if (
    ...
    !isNaN(parseFloat(numberOrString))
  ) {
...
export function extractMetadataInfo(metadata) {
  ...
  const feeTierMatch = metadata?.description
    ? metadata.description.match(/Fee Tier: ([0-9.]+%)/)
    : "Unknown";
  const feeTier = feeTierMatch ? parseFloat(feeTierMatch[1]) : null;

  // Extract pairName using regular expression
  const pairNameMatch = metadata?.name
    ? metadata.name.match(/Uniswap - ([0-9.]+%) - (.+?) -/)
    : "Unknown";
  ...
  const poolAddressMatch = metadata?.description
    ? metadata.description.match(/Pool Address: ([0-9a-fA-Fx]+)/)
    : "Unknown";
  const poolAddress = poolAddressMatch ? poolAddressMatch[1] : null;

  const tokenAddressesMatch = metadata?.description
    ? metadata.description.match(/([a-zA-Z0-9]+) Address: ([0-9a-fA-Fx]+)/g)
    : "Unknown";
  const tokenInfo = [];
  ...
  data.forEach((item) => {
    if (item.last_transaction && item.last_transaction.block_timestamp) {
      const timestamp = new Date(
        item.last_transaction.block_timestamp
      ).getTime();
...
export function uniqueAddressesFromTransaction(transaction) {
  ...
  if (transaction.from_address) {
    if (transaction.from_address_label) {
      addresses.add(
        `${transaction.from_address} ${transaction.from_address_label}`
      );
    } else {
  ...
  if (transaction.to_address) {
    if (transaction.to_address_label) {
      addresses.add(
        `${transaction.to_address} ${transaction.to_address_label}`
      );
    } else {
    ...
    transaction.token_transfers.forEach((transfer) => {
      if (transfer.from_address) {
        if (transfer.from_address_label) {
          addresses.add(
            `${transfer.from_address} ${transfer.from_address_label}`
          );
        } else {
    ...
    transaction.approvals.forEach((approval) => {
      if (approval.token.token_address) {
        if (approval.token.token_address_label) {
          addresses.add(
            `${approval.token.token_address} ${approval.token.token_address_label}`
          );
        } else {
      ...
      if (approval.spender.address) {
        if (approval.spender.address_label) {
          addresses.add(
            `${approval.spender.address} ${approval.spender.address_label}`
          );
        } else {
...
export function checkLiquidityLocked(providers) {
  let liquidityLocked = false;
  let amountLocked = 0;
  let amountLockedPercentage = 0;
  if (!providers || providers.length === 0) {
    return {
      liquidityLocked,
      amountLocked: parseFloat(amountLocked.toFixed(4)), // formatted to 4 decimal places
      amountLockedPercentage: parseFloat(amountLockedPercentage.toFixed(2)), // formatted to 2 decimal places
    };
  }

  providers.forEach((provider) => {
    // Check if the owner address contains "0x00000000000000000000000000"
    if (provider.owner_address.includes("0x00000000000000000000000000")) {
      liquidityLocked = true;
      amountLocked += parseFloat(provider.balance_formatted);
      amountLockedPercentage += provider.percentage_relative_to_total_supply;
    }
  });
