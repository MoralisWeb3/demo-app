  ...
  try {
    ...
    do {
      const response = await fetch(
        `${baseURL}/wallets/${address}/tokens?${
      ...
      if (tokens.result) {
        for (let item of tokens.result) {
          all_tokens.push(item);
        }
      }
    ...
    do {
      const nfts_response = await fetch(
        `${baseURL}/${address}/nft/collections?chain=${chain}&cursor=${cursor}&limit=300`,
        {
          method: "GET",
          headers: {
            Accept: "application/json",
